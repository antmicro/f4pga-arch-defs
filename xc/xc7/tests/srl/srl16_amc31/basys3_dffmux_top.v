...
module top (
  ...
  generate
    for (i = 0; i < SRL_COUNT; i = i + 1) begin
      ...
      srl_shift_tester #(
          .SRL_LENGTH (17),  // One extra for DFF reg
          .FIXED_DELAY(17)
      ) tester (
          .clk   (clk),
          .rst   (rst),
          ...
          .srl_sh(srl_sh),
          .srl_d (srl_d),
          .srl_q (srl_q15[i] ^ sim_error),
          .srl_a (),
          .error (error[i])
      );

      always @(posedge clk) if (srl_sh) srl_q15[i] <= srl_q15_o;
