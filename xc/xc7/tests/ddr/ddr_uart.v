...
module top (
  ...
  reg [9:0] memadr;
  always @(posedge sys_clk) begin
    if (main_sram_we[0]) mem[main_sram_adr][7:0] <= main_sram_dat_w[7:0];
    if (main_sram_we[1]) mem[main_sram_adr][15:8] <= main_sram_dat_w[15:8];
    if (main_sram_we[2]) mem[main_sram_adr][23:16] <= main_sram_dat_w[23:16];
    if (main_sram_we[3]) mem[main_sram_adr][31:24] <= main_sram_dat_w[31:24];
