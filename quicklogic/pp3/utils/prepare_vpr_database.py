...
def process_tilegrid(
    ...
    for tile_type_name in assp_types:
        ...
        # Set ASSP VPR tile base location
        assp_loc = {
...
def process_connections(
    ...
    for i, connection in enumerate(cur_connections):
        ...
        for j, ep in enumerate(eps):
            ...
            # Get the original cell type of the connection
            ...
            if cell_type not in special_cell_types:
                continue

            # Check if this connection refers to a pin of a split ASSP tile
            if cell_type in assp_split_rules:
                ...
                for tile_rule in assp_split_rules[cell_type]:
                    ...
                    for cell_rule in tile_rule["cells"]:
                        ...
                        if pin in pin_map:
                            ep_pin = cell_name + "_" + pin_map[pin]
                            ep_loc = Loc(
                                x = ep.loc.x + offset[0],
                                y = ep.loc.y + offset[1],
                                z = ep.loc.z + offset[2],
            ...
            # This is not part of a split ASSP
                ...
                # Look for the cell type the connection refers to
                cells = [cell for cell in phy_tile.cells \
                         if cell.type == cell_type]
                assert cells

                # Remap cell name if applicable
                cell_name = assp_name_map.get(cells[0].name, cells[0].name)

                # Remap the endpoint
                ep_pin = ep.pin
                ep_loc = special_cell_loc[cell_type][cell_name]
